...
<script type="text/javascript">
	...
	$(function(){
            ...
            $.ajax({
                ...
                success:function (data) {
                	if(data.code==0){
						...
						var html = '';
                		html+="<div class=\"remarkDiv\" id=\"div_"+data.remark.id+"\" style=\"height: 60px;\">";
						html+="<img title=\"${sessionScope.user.name}\" src=\"image/user-thumbnail.png\" style=\"width: 30px; height:30px;\">";
						html+="<div style=\"position: relative; top: -40px; left: 40px;\" >";
						html+="<h5>"+noteContent+"</h5>";
						html+="<font color=\"gray\">客户</font> <font color=\"gray\">-</font> <b>${customer.name}</b> <small style=\"color: gray;\">"+data.remark.createTime+" 由${sessionScope.user.name}创建</small>";
						html+="<div style=\"position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;\">";
						html+="<a class=\"myHref\" name=\"editA\" remarkId="+data.remark.id+" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-edit\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>";
						html+="&nbsp;&nbsp;&nbsp;&nbsp;";
						html+="<a class=\"myHref\" name=\"deleteA\" remarkId="+data.remark.id+" href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-remove\" style=\"font-size: 20px; color: #E6E6E6;\"></span></a>";
						html+="</div>";
						html+="</div>";
						html+="</div>";
                ...
                $.ajax({
                    ...
                    success:function (data) {
                        ...
                        $.each(data,function (index,obj) {
                            array.push(obj.name);//往数组中存放数据
                            name2id[obj.name]=obj.id//给对象 name2id设置属性名和属性值
                        });
                        process(array);//把数据交给source使用，完成自动补全
                    }
                })
        ...
        $("#createContactsBtn").click(function () {
            ...
            $.ajax({
                url:"workbench/customer/saveCreateContacts.do",
                data:{
                    customerId:customerId,
                    owner:owner,
                    source:source,
		...
		//给删除联系人按钮绑定单击事件
		$("#ContactsListDiv").on("click","a[name=deleteContacts]",function () {
        ...
        //给交易删除按钮绑定事件
		$("#TranBody").on("click","a[name=deleteTran]",function () {
			$("#removeTransactionModal").modal("show");
			var id=$(this).attr("tran-id");
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 30%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button  id="deleteContactsBtn" type="button" class="btn btn-danger" data-dismiss="modal">删除</button>
				</div>
			</div>
		</div>
	</div>
    ...
    <div class="modal fade" id="removeTransactionModal" role="dialog">
        <div class="modal-dialog" role="document" style="width: 30%;">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button id="deleteTransactionBtn" type="button" class="btn btn-danger" data-dismiss="modal">删除</button>
                </div>
            </div>
        </div>
    </div>
		...
		<div class="modal-dialog" role="document" style="width: 85%;">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" role="form">
						...
						<div class="form-group" style="position: relative;">
							...
							<div class="col-sm-10" style="width: 300px;">
                                <input hidden id="create-customerId">
				...
				<div class="modal-footer">
					...
					<button id="createContactsBtn" type="button" class="btn btn-primary" data-dismiss="modal">保存</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; top: 10px; left: 40px;" id="remarkDivList">
        ...
        <c:if test="${not empty customerRemarkList}">
            <c:forEach items="${customerRemarkList}" var="crl">
                <div class="remarkDiv" id="div_${crl.id}" style="height: 60px;">
                    <img title="${crl.createBy}" src="image/user-thumbnail.png" style="width: 30px; height:30px;">
                    <div style="position: relative; top: -40px; left: 40px;" >
                        <h5>${crl.noteContent}</h5>
                        <font color="gray">客户</font> <font color="gray">-</font> <b>${customer.name}</b> <small style="color: gray;">${crl.editFlag==0?crl.createTime:crl.editTime} 由${crl.editFlag==0?crl.createBy:crl.editBy}${crl.editFlag==0?"创建":"修改"}</small>
                        <div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">
                            <a class="myHref" name="editA" href="javascript:void(0);"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            <a class="myHref" name="deleteA" href="javascript:void(0);"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>
                        </div>
                    </div>
                </div>
            </c:forEach>
			...
			<div style="position: relative;top: 0px;">
				<table id="activityTable2" class="table table-hover" style="width: 900px;">
                    ...
                    <c:if test="${not empty transactionList}">
                        <c:forEach items="${transactionList}" var="tl">
                            <tr id="tl_${tl.id}">
                                <td><a href="transaction/detail.jsp" style="text-decoration: none;">${tl.name}</a></td>
                                <td>${tl.money}</td>
                                <td>${tl.stage}</td>
                                <td>${tl.possibility}</td>
                                <td>${tl.expectedDate}</td>
                                <td>${tl.type}</td>
                                <td><a href="javascript:void(0);" name="deleteTran" tran-id="${tl.id}" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>
                            </tr>
                        </c:forEach>
                    </c:if>
						<%--<tr>--%>
				...
				<table id="activityTable" class="table table-hover" style="width: 900px;">
					...
					<tbody>
                        ...
                        <c:if test="${not empty contactsList}">
                            <c:forEach items="${contactsList}" var="col">
                                <tr id="ct_${col.id}">
                                    <td><a href="contacts/detail.html" style="text-decoration: none;">${col.fullname}</a></td>
                                    <td>${col.email}</td>
                                    <td>${col.mphone}</td>
                                    <td><a href="javascript:void(0);" name="deleteContacts" contactsId="${col.id}" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>
                                </tr>
                            </c:forEach>
                        </c:if>
						<%--<tr>--%>
